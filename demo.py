...
def insert_embedding(c,np,model):
    new_embedding=[c]
    for t in np:
        tmp=c[0].clone()
        start=t[1]+1
        length=t[2]
        emb=model.get_learned_conditioning([t[0]])
        tmp[:,start:start+length,:]=emb[:,1:1+length,:]
        new_embedding.append(len(c)*[tmp])
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        ...
                        if opt.parser_type=='constituency':
                            nps=get_noun_phrases(nlp,prompts[0])
                        else:
                            raise NotImplementedError
                        noun_phrases=[[text[0]]*len(prompts) for text in nps]
                        np_embeddings=[model.get_learned_conditioning(np) for np in noun_phrases]
                        if opt.conjunction:
                            pass
                        else:
                            key=c
                            value=insert_embedding(c,nps,model)
                            c={'k':key,'v':value}
